...
\abstract{
This paper investigates the possibility of creating a system that can automatically port applications written for Apple devices to applications that can run on Android devices. This is done by developing a prototype system that can translate a small subset of applications and explaining how it can be extended to a fully functioning system. The prototype system is described in high detail and its correctness and performance are evaluated. The results are that it produces programs that perform relatively good, when compared to manually ported programs. The author argues that the prototype can be successfully extended to produce a complete compiler that would also produce programs of good quality. He concludes that producing such a tool would be beneficial to developers who want to reach a wider audience and simplify their development procedures, and would have a positive effect on the development communities by bringing them together.
}
